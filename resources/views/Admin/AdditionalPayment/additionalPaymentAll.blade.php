...
<p class="text-xl">مجموع کل اضافه پرداختی‌ها: {{$additionalpayments }} افغانی</p>
</div>
    <form action="{{ route('admin.getPaidAdditionalPayments', ['userId' => $userId]) }}" method="GET" class="mb-6 flex flex-col md:flex-row items-center md:space-x-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <a href="{{ route('admin.singleAdditionalPayment', ['id' => $payment->id]) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">ویرایش</a>

                <form action="{{ route('admin.deleteAdditionalPayment', ['id' => $payment->id]) }}" method="POST" class="delete-user-form">
                    @csrf
                    @method('DELETE')
